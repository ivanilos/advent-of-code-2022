...
func calc(valveName string, visitedMask int, timeLeft int,
	interestingValves []string, distances [][]int, valves map[string]Valve,
	dp map[State]int) int {

	state := State{valveName, visitedMask, timeLeft}
	...
	dp[state] = 0
	curValveID := valveIDToInt(valveName)
	for i := 0; i < len(interestingValves); i++ {
		if !visited(visitedMask, i) {
			...
			if timeLeft-dist-1 >= 0 {
				nextTimeLeft := timeLeft - dist - 1
				score := nextTimeLeft * valves[nextValve].flow
				nextVisitedMask := visitedMask | (1 << i)

				aux := score + calc(nextValve, nextVisitedMask, nextTimeLeft,
...
func solve(valves map[string]Valve) int {
	...
	dp := map[State]int{}
	return calc("AA", 0, totalTime, interestingValves, distances, valves, dp)
}
